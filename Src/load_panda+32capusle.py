...
XML_PATH = r"d:\DVRK\MUJOCO\Assest\franka_emika_panda\scene32.xml"
...
# ---------- 参数 ----------
EE_BODY     = "hand"
TCP_OFFSET  = np.array([0.0, 0.0, 0.1034])   # hand 局部系 -> 两指尖中点

POS_STEP    = 0.0008   # 每步笛卡尔位移 (m)
GRIP_STEP   = 3        # 每步夹爪控制量
JOINT7_STEP = 0.005    # 每步 joint7 角度增量 (rad)
DAMPING     = 1e-2     # 阻尼系数（加大，让 IK 更保守）
ORI_GAIN    = 0.3      # 姿态纠偏增益
MAX_DQ      = 0.002    # 每步单关节最大变化量 (rad)，防止暴冲
...
def vertical_orientation_error(data, ee_id):
    """
    计算使夹爪严格垂直向下所需的旋转误差向量。
    目标姿态：hand 的 Z 轴 = [0,0,-1]（向下），
    X/Y 轴保持当前绕竖直轴的投影（由 Z/X 键控制）。
    """
    R_cur = data.xmat[ee_id].reshape(3, 3)

    z_des = np.array([0.0, 0.0, -1.0])

    # 由当前 X 轴水平投影得到期望 X 轴（保留绕竖直轴的朝向）
    x_proj = R_cur[:, 0].copy()
    x_proj[2] = 0.0
...
def diff_ik(model, data, ee_id, dpos):
    ...
    每一步都主动将夹爪姿态修正为垂直向下：
      - 位置误差 = dpos（键盘输入）
      - 旋转误差 = 当前姿态 vs 竖直朝下目标 × ORI_GAIN
    即使没有按键，只要姿态有偏差就会自动纠正。
    """
...
def main():
    ...
    # 找到圆环 freejoint 在 qpos 中的地址，用于 reset 后恢复
    ...
    # model.qpos0 保存了 XML 中 body pos 对应的初始值
    torus_init_qpos = model.qpos0[torus_qadr:torus_qadr + 7].copy()

    # 初始化：加载 home keyframe（机械臂）+ 恢复圆环位置
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            # 检测 viewer Reset（time 回零）=> 恢复 ctrl + 圆环位置
