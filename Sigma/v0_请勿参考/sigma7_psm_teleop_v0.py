...
PIVOT_BODY = "tool_wrist"
PIVOT_OFFSET_LOCAL = np.array([0.0, 0.0, 0.0], dtype=float)
...
def compute_ik_step(
    ...
    pos_task_weight: float,
    ori_task_weight: float,
    ...
    current_rotation = body_rotation(data, body_id)
    ...
    reg = damping * np.diag(joint_damping_scale)
    if track_orientation:
        angular_error = orientation_gain * rotation_error(current_rotation, target_rotation)
        Jr = jacr[:, dof_columns]
        wp = float(max(pos_task_weight, 1e-6))
        wr = float(max(ori_task_weight, 1e-6))
        lhs = wp * (Jp.T @ Wp @ Jp) + wr * (Jr.T @ Jr) + reg
        rhs = wp * (Jp.T @ Wp @ position_error) + wr * (Jr.T @ angular_error)
    else:
        lhs = Jp.T @ Wp @ Jp + reg
        rhs = Jp.T @ Wp @ position_error
...
def make_calibration(
    ...
    return TeleopCalibration(
        ...
        target_rotation_home=body_rotation(data, body_id),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--ik-wrist-damping-scale",
        type=float,
        default=2.0,
        help="Extra IK damping on roll/wrist_pitch/wrist_yaw vs yaw/pitch/insertion (>=1). "
        "Larger = prefer base joints for translation so pitch participates.",
    )
    parser.add_argument(
        "--ik-pos-weight",
        type=float,
        default=12.0,
        help="IK task weight on Cartesian position vs orientation (higher = yaw/pitch follow xy/z more)",
    )
    parser.add_argument(
        "--ik-ori-weight",
        type=float,
        default=1.0,
        help="IK task weight on orientation; lower relative to --ik-pos-weight reduces wrist hogging dq",
    )
    parser.add_argument(
        "--ik-pos-weight-x",
        type=float,
        default=4.0,
        help="Extra Cartesian task weight on world X translation",
    )
    parser.add_argument(
        "--ik-pos-weight-y",
        type=float,
        default=4.0,
        help="Extra Cartesian task weight on world Y translation",
    )
    parser.add_argument(
        "--ik-pos-weight-z",
        type=float,
        default=1.0,
        help="Extra Cartesian task weight on world Z translation",
    )
    parser.add_argument(
        "--master-planar-yaw-deg",
        type=float,
        default=0.0,
        help="Rotate master (sigma) x/y/z displacement about world +Z before adding to pivot target (deg). "
        "Tune if device axes are misaligned with MuJoCo world so x/y motion uses pitch as expected.",
    )
    parser.add_argument("--orientation-gain", type=float, default=0.18, help="IK orientation error gain")
    parser.add_argument(
        "--no-orientation",
        action="store_true",
        help="Stabilize teleop by tracking position only (ignore master orientation)",
    )
    ...
    parser.add_argument("--deadband-rot", type=float, default=2e-3, help="Orientation deadband (norm) in radians")
...
def main() -> None:
    ...
    if pivot_body_id < 0:
        raise ValueError(f"MuJoCo body not found: {PIVOT_BODY}")
    ...
    s_w = float(max(args.ik_wrist_damping_scale, 1.0))
    ik_joint_damping_scale = np.array([1.0, 1.0, 1.0, s_w, s_w, s_w], dtype=float)
    position_axis_weights = np.array(
        [
            max(args.ik_pos_weight_x, 1e-6),
            max(args.ik_pos_weight_y, 1e-6),
            max(args.ik_pos_weight_z, 1e-6),
        ],
        dtype=float,
    )
    ...
    try:
        ...
        device.open()
        calibration = make_calibration(device, model, data, pivot_body_id, PIVOT_OFFSET_LOCAL, arm_actuators, jaw_actuator)
            ...
            while viewer.is_running():
                ...
                if command == "r":
                    calibration = make_calibration(
                        device, model, data, pivot_body_id, PIVOT_OFFSET_LOCAL, arm_actuators, jaw_actuator
                    )
                    target_position_filt = calibration.target_position_home.copy()
                    dq_filt[:] = 0.0
                    ctrl_filt = data.ctrl[arm_actuators].copy()
                ...
                current_position = point_world_pos(data, pivot_body_id, PIVOT_OFFSET_LOCAL)
                current_rotation = body_rotation(data, pivot_body_id)
                e_pos = target_position_filt - current_position
                e_rot = rotation_error(current_rotation, target_rotation)
                if float(np.linalg.norm(e_pos)) < float(args.deadband_pos) and (
                    (not track_orientation) or float(np.linalg.norm(e_rot)) < float(args.deadband_rot)
                ):
                ...
                dq = compute_ik_step(
                    model=model,
                    data=data,
                    body_id=pivot_body_id,
                    offset_local=PIVOT_OFFSET_LOCAL,
                    ...
                    joint_damping_scale=ik_joint_damping_scale,
                    pos_task_weight=args.ik_pos_weight,
                    ori_task_weight=args.ik_ori_weight,
                    position_axis_weights=position_axis_weights,
                )
                dq_filt = dq_filt + float(args.dq_smooth) * (dq - dq_filt)
                # Make insertion less aggressive and more smoothed to avoid chatter.
                if len(dq_filt) >= 3:
                    dq_filt[2] = dq_filt[2] + float(args.dq_smooth_ins) * (dq[2] - dq_filt[2])
                    dq_filt[2] *= float(args.insertion_dq_scale)
                ...
                if now - last_print >= args.print_every:
                    last_print = now
                    pivot = point_world_pos(data, pivot_body_id, PIVOT_OFFSET_LOCAL)
                    print(
                        "\r"
                        f"sigma=({master_position[0]:+.3f},{master_position[1]:+.3f},{master_position[2]:+.3f}) m "
                        f"pivot=({pivot[0]:+.3f},{pivot[1]:+.3f},{pivot[2]:+.3f}) m "
